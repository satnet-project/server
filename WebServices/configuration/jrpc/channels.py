...
def gs_channel_get_configuration(ground_station_id, channel_id):
    ...
    return get_gs_channel_configuration(ch)
...
@rpcmethod(name='configuration.sc.channel.getOptions',
           signature=['String', 'Object'], login_required=True)
def sc_channel_get_options():
    """
    JRPC method.
...
@rpcmethod(name='configuration.sc.channel.isUnique',
           signature=['String'], login_required=True)
...
@rpcmethod(name='configuration.sc.channel.create',
           signature=['String', 'String', 'Object'], login_required=True)
def sc_channel_create(spacecraft_id, channel_id, configuration):
    ...
    frequency, modulation, bitrate, bandwidth, polarization\
        = get_sc_channel_parameters(configuration)
    ...
    # the related objects (channel parameters). Therefore, in case one of the
    # given objects does not exist, an exception will be raised.
    SpacecraftConfiguration.objects.add_channel(
