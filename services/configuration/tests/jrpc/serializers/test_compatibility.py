...
class TestCompatibilitySerializers(test.TestCase):
    ...
    def test_compatibility_serializers(self):
        ...
        a_tuples = compatibility_serializers.CompatibilitySerializer\
            .serialize_gs_ch_compatibility_tuples(test_tuples)

        self.assertEquals(
            a_tuples[0]['groundstation']['identifier'], 'gs-castrelos',
            "Wrong gs identifier!"
        )
        self.assertEquals(
            a_tuples[0]['gs_channel']['identifier'], 'chan-cas-1',
            "Wrong gs channel identifier!"
        )
